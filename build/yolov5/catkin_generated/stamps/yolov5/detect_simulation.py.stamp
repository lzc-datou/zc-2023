        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    rospy.loginfo("yolov5 pub successufully")
                    # 两个列表清空，用于下一次循环使用
...
if __name__ == '__main__':
    ...
    opt = parse_opt()
    # 删除了源码detect.py中几处保存检测结果的代码，因为现在是检测ROS话题中的图像，如果每一张图像都保存的话占用的空间过大
    # 初始化ros节点
    ...
    rospy.Subscriber("/downward_camera/image_raw",Image,img_callback,queue_size=10)
    rospy.spin()
