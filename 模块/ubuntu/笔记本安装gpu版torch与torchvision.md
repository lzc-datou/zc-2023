...
2. 分别下载版本相互匹配的torch与torchvision。见[网站]。(https://download.pytorch.org/whl)。该网站不好使时用下面两个。
    1. [torch](https://download.pytorch.org/whl/torch/)
    2. [torchvision](https://download.pytorch.org/whl/torchvision/)  
    值得注意的是torch与torchvision的命名方式。
